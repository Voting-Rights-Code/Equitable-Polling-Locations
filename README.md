...
1. Compute the distance from the centroid of each census block to each potential polling location (building or best case scenario)
...
1. Compute the Kolm-Pollak weight from each block group to each polling location
    1. KP_factor  = $e^{-beta * alpha * distance}$
        1. *beta* is a user-defined parameter that indicates the user's aversion to inequality (more about beta later)
        1. *alpha* is a data-derived normalization factor: $alpha = \frac{\sum population * distance}{\sum population * distance^2}$, where *distance* is the distance of each census block to its closest _existing_ poll
    1. The KP_factor plays the role of a weighted distance in a standard objective function.
        ...
        1. For instance, a group of 5 people all having to travel 1 mile to a polling location would have a lower KP_factor than a situation where 4 people travel 1/2 a mile while the 5th travels 3 miles, even though the average distance traveled in both cases is the same. (When the distances are all the same, the KP_factor is equal to the mean distance. The more unequal the distribution of distances, the higher above the mean the KP_factor is.)
1. Choose whether to minimize the average distance or the inequity penalized score (y_EDE) in the model 
    1. Set *beta* = 0 for average distance
        1. In this case, minimize the average distance traveled: minimize $\frac{\sum (block~population)*distance}{county~population}$
    1. Set *beta* in $[-2, 0)$ to minimize Kolm-Pollak score (y_EDE) 
        1. The lower the *beta*, the greater penalty to inequality
        1. In this case: minimize $\frac{\sum (block~population) * \text{KP_factor}}{county~population}$
1. Minimize the above while respecting the following constraints:
    1. Open a user-specified number of polling locations
    1. Maintain a user-defined bound on the number of new (verses existing) locations
...
* In the directory of the Equitable-Polling-Locations git repo:
    * python ./model_run_cli.py -cNUM -lLOG_FOLDER ./path/to/config/file.yaml
        ...
        * LOG_FOLDER = Folder to store log files 
            * This folder must exist or code will not run
        * path to config file accepts wildcards to set of sequential runs

From Google Colab:
* For example, follow the the instructions in [this file](./colab_Gwinnett_expanded_multi_11_12_13_14_15.ipynb) (To be accessed in the directory of the Equitable-Polling-Locations git repo)
# Input files
...
* Ethnicity (Hispanic / Non-Hispanic) is orthogonal to race in the census data. Therefore, one may be Hispanic and Asian at the same time.
...
* Instructions for downloading this data:
    ...
    * Select Geography:
        * Using the menu on the far left: filter for Geography -> Block -> State -> County Name, State -> All Blocks within County Name, State
        * If asked to select table vintage, select 2020;  DEC Redistricting Data (PL-94-171)
    * Unzip and place the contents of the downloaded folder in 'datasets/census/redistricting/County_ST/' (e.g., County_ST = Gwinnett_GA)
    * (Unzipped folder may contain additional files: keep 'DECENNIALPL2020.P3-Data.csv')
* Columns used from P3:
    * White alone
...
* This another census dataset for a racial breakdown of people of voting age by census block, but focused only on Hispanic / Latino or not.
...
* Instructions for downloading this data:
    ...
    * Select Geography:
        * Filter for Geography -> Block -> State -> County Name, State -> All Blocks within County Name, State
        * If asked to select table vintage, select 2020;  DEC Redistricting Data (PL-94-171)
    * Unzip and place the contents of the dowloaded folder in 'datasets/census/redistricting/County_ST/'
    * (Unzipped folder may contain additional files: keep 'DECENNIALPL2020.P4-Data.csv')
* Columns used from P4:
    * Total population
    * Total hispanic
    * Total non-hispanic
### **datasets/census/tiger/County_ST/tl_censusYYYY_FIPS_tablockcensusYY.shp**:
(for example: 'datasets/census/tiger/Gwinnett_GA/tl_2020_13135_tabblock20.shp')
...
* Instuction for downloading this data:
    ...
    * Download tl_censusYYYY_FIPS_tablockcensusYY.zip (e.g. 'tl_2020_13135_tablock20.zip')
    * Unzip and place the contents of the dowloaded folder in 'datasets/census/tiger/County_ST/'
...
### **datasets/census/tiger/County_ST/tl_cenesusYYYY_FIPS_bgcenesusYY.shp**:
The instructions for downloading this data is identical the instructions for the blocks with the following exception:
* Download tl_censusYYYY_FIPS_bgcensusYY.zip (e.g. 'tl_2020_13135_bg20.zip')
...
| ----- | ------ | ----- |
| Location type | If potential location, has a 'location type' category and the word 'Potential' (case sensitive). Previous precincts have precinct type and election year(s). | 'Community Center - Potential', 'EV_2020_2022' | 
| ----- | ------ | ----- |
| Lat, Long | Comma separated concatenation of latitude and longitude (can be copied from google maps by right clicking on the location marker for the address) | '33.964717796407434, -83.85827288222517' |
| ----- | ------ | ----- |
...
Example path: 'Gwinnett_GA_configs/Gwinnett_config_full_11.yaml'
  * Mandatory arguments in a config (yaml) file
    * location: County_ST 
        * e.g. 'Gwinnett_GA'
        * This variable is used throughout to name files
    * year: List of years one wants to consider actual polling locations for. 
        * e.g. ['2022', '2020'] 
    * level: one of 'original', 'expanded', 'full'
      * 'original': Use if you just want to reassign people more optimally to existing polling locations. Useful for approximating current geographic access.
      ...
      * 'full': Includes the census block group centroids as potential locations. Use if you want a more ideal list of locations, for instance, to understand where to look for potential polling locations that have yet to be identified.
    * beta: number in [-2, 0]. 
        * Aversion to inequality. 
            * If 0, model minimizes the mean distance. 
            * The further below 0, the greater the aversion to inequality. 
    * time_limit: maximal number of seconds that the optimizer will run 
    * capacity: >= 1. A multiplicative factor that indicates how much more than $\frac{county~population}{precincts~open}$ each precint is allowed to be alloted

  * Optional arguments
    * precincts_open: number of precints to be assigned
        * Default: number of existing polling locations
    * max_min_mult: number >= 1 
        * A scalar to limit the search radius to match polling locations. If this is too small, may not have a solution. 
        * Default: 1
    * maxpctnew: number in [0,1]
        * The percent of new locations allowed to be selected
        * Default: 1 
    * minpctold: number in [0,1]
        * The percent of existing locations allowed to be selected
        * Default: 0

# Intermediate dataset

### **datasets/polling/County_ST/County_ST.csv**: 
This is the main data set that the optimizer uses. It includes polling locations from previous years, potential polling locations, and block group centroids, as well as distances from block centroids (representing residences) to the above.
...
| ----- | ------ | ----- | ----- |
|id_dest | Name of the actual or potential polling location | 'Location' from County_ST_locations_only.csv | 'Bethesda Senior Center' |
| ----- | ------ | ----- | ----- |
...
* If the file was run via command line, the outputs are written in the folder County_ST_results/
    * The output files have the names:
...
* *_edes.csv (demographic level ede scores)
    * For each demographic group (asian, black, hispanic, native, population, white), this table records the 
        ...
        * avg_dist: average distance traveled by the members of that demographic: weighted_distance / demo_pop
        * y_EDE: the Kolm-Pollak score (y_EDE) for the demographic: $\frac{-1}{beta* alpha}\log(\text{avg_KP_weight})$ 
            * where avg_KP_weight = $\frac{\sum demo\_res\_obj\_summand}{total~demo\_pop}$
* *_precinct_distances.csv (distances traveled to each precint by demographic)
    * For each demographic group (asian, black, hispanic, native, population, white), and selected polling location (id_dest), this table records the 
