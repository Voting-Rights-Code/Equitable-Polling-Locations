...
Example path: Gwinnett_County_GA_configs/Gwinnett_config_full_11.yaml

Recommended convention: Each config folder should only have one parameter changing. For example, DeKalb_County_GA_no_bg_school_config should contain only (and all) runs with block groups and schools in the bad list, changing only the number of desired polling locations
  * Mandatory arguments
