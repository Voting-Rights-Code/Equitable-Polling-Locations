...
`R/result_analysis/scripts/Basic_analysis.r` runs the core set of graphs, regression and maps. This script is set up to compare historical polling location data with potential polling configurations. However, given the correct configurations, it can run for just historical data.
...
1. From terminal, run `Rscript R/result_analysis/scripts/Basic_analysis.r name_of_config.r`. Note, name_of_config is only the file name, not the path. E.g `Berkeley_County_original.r`.
...
**Access:** the analysis can write to any Google Cloud Storage bucket you have write access to — point `STORAGE_BUCKET` at one you control. If you'd prefer to use the Voting Rights Code shared `equitable-polling-analysis` bucket, reach out to a Voting Rights Code team member and we can grant access on request. (Read access to the project's BigQuery datasets is a separate grant; see [Database Access](database.md#database-access).)
