...
rm(x)
options(GMAP_Tile=TRUE)   # display gmaps
options(OSM_reCalc=TRUE)  # recalculate OSM drivng distance 
```

```{r}
#| label: Functions
# OSRM API query to get the travel geometry between two points. This function interfaces with the route OSRM service.
osrm_route <- function(orig_lon, orig_lat, dest_lon, dest_lat,returnlevel) {
  ...
  # Get route
  route = osrmRoute(src = orig, dst = dest, overview = returnlevel,osrm.profile="car")
  
  # 
  if (returnlevel  %in% c("full","simplified")) {
      return(route)              #return entire list
  } else if (!returnlevel) {     #false 
    return( list(route))         # return time and distance to data table 
  }
}
...
make_osrm_map <- function(orig_lon,orig_lat,dest_lon,dest_lat){
  ...
  route <- osrm_route(orig_lon,orig_lat,dest_lon,dest_lat,"full")
  ...
  map<-leaflet() |> addTiles() |>
    addMarkers(lng = orig_lon, lat = orig_lat, popup = paste("Origin - Coordinates:",orig_lon,",",orig_lat) ) |>
    addMarkers(lng = dest_lon, lat = dest_lat, popup = paste("Destination - Coordinates:",dest_lon,",",dest_lat)) |>
...
#calculate the difference between the the map directions. 
distance_compare[, distance_difference := abs(OSM_distance_m  -  GMAP_distance_m)]   
...
if (getOption("OSM_reCalc")) {
  # Optionally calculate the OSM differences since there is a difference between provided and calculated values
  
  #calculate the driving distances from the server
  distance_compare[,result:=(mapply(osrm_route,orig_lon,orig_lat,dest_lon,dest_lat,FALSE))][
                 , c("OSM_distance_m") :=    sapply(result, function(x)  x[2]*1000)][
                 , result:=NULL]

  #use the new driving data to recalculate the differences between the programs. 
  distance_compare[, distance_difference :=  abs(as.numeric(OSM_distance_m)  -  GMAP_distance_m)]
}
...
#| echo: false
#select the 2 records with the biggest discrepancy in distance for each sample group.
...
#OSM distances recalculated -- use those variables:
