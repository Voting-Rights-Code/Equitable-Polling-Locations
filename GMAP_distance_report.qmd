...
x=grep("Platform",readLines(paste0(root,"/authentication_files")), value=TRUE)
...
# Extract routes from the returned info
  ...
  #If GMAP_Tile is True the display directions on Google Maps tile
    ...
    #register_google(key =getOption("GMAP_API_KEY"))
    ...
    suppressMessages({
        map <- get_googlemap(center = c(lon = mean(route_df$lon), lat = mean(route_df$lat)),
                          zoom = 11, maptype = "roadmap",key=getOption("GMAP_API_KEY"))  |> 
