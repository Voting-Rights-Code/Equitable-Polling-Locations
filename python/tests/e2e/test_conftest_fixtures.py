...
class TestE2eTestDataFiles:
    ...
    def test_all_config_variants_exist(self, e2e_test_data):
        """Each entry in CONFIG_VARIANTS must have a corresponding YAML file on disk."""
    ...
    def test_config_variant_count(self, e2e_test_data):
        """Exactly one config file is created per CONFIG_VARIANTS entry."""
        assert len(e2e_test_data['configs']) == len(CONFIG_VARIANTS)
