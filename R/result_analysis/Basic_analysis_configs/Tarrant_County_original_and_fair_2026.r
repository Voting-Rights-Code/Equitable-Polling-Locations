...
ORIG_CONFIG_FOLDER = "Tarrant_County_TX_original_configs_capacity_2"
POTENTIAL_CONFIG_FOLDER = "Tarrant_County_TX_fair_capacity_2" #leave NULL if only want historical analysis
ORIG_FIELD_OF_INTEREST = '' #must not leave empty if config set has only one element
POTENTIAL_FIELD_OF_INTEREST = 'precincts_open' #must not leave empty if config set has only one element
...
IDEAL_POLL_NUMBER  = 215 #the optimal number of polls desired for this county
...
DESCRIPTOR_DICT_ORIG <- c('year_2024' = '2024', 'year_2025' = '2025', 
                     'year_2026prop' = 'proposed', 'year_2026act' = '2026'
                     )
DESCRIPTOR_DICT_POTENTIAL <- c('precincts_open_215' = '215_open' 
                     )
