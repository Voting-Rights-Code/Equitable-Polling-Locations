...
# Extract routes from the returned info
  ...
  #If GMAP_Tile is False the display directions on OSM tile
  ...
  #If GMAP_Tile is True the display directions on Google Maps tile
    ...
    #register_google(key =getOption("GMAP_Directions_API_KEY"))
    ...
    suppressMessages({
        map <- get_googlemap(center = c(lon = mean(route_df$lon), lat = mean(route_df$lat)),
                          zoom = 11, maptype = "roadmap",key=getOption("GMAP_Directions_API_KEY"))  |> 
...
distance_compare[, distance_difference := abs(program_distance_m)]   #difference calculated in python program for efficency 
