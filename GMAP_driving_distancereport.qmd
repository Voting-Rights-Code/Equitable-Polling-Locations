...
```{r}
#| label:  Set Constants & OSRM servers and keys
#| echo: false
#| output: false 

# define files
path = '/mnt/bin/BlueBonnet/Project_Archive/Equitable-Polling-Locations/datasets/driving'
...
x=grep("Directions",readLines("/mnt/bin/BlueBonnet/Project_Archive/Equitable-Polling-Locations/authentication_files"), value=TRUE)
options(GMAP_Directions_API_KEY=gsub('GMAP_Directions_API_KEY=','',x))
rm(x)
```
...
make_google_map <- function(orig_lon,orig_lat,dest_lon,dest_lat,api_key){
  # Using Google maps map the driving directions between two points
  # Define origin and destination
...
## Reverse Geocoding 
# Function to reverse geocode using local Nominatim
...
table_difference <- function(dt_diff){
  gt(dt_diff,groupname_col = 'grp_label') |>
    ...
    cols_move(
      columns = c("dest","distance_m"),after = orig
    ) |>
    ...
    tab_header(
      title = 'Two Largest Driving Distance Variations Between Google Maps and OSM',
...
```{r}
#| label: read data
distance_compare <- fread(file = paste0(path,'/',location,'/',"Virginia_Beach_City_VA_compare_driving_distances.csv"))
distance_sample <- fread(file = paste0(path,'/',location,'/',"Virginia_Beach_City_VA_sampling_info_driving_distances.csv"))
distance_compare[, distance_difference := abs(GMAP_distance_m - OSM_distance_m)]
distance_compare<-distance_compare[, `:=` (sort_order=fcase(Sample_Block=='inc' , 2, 
                                           ...
                                           grp_label= fcase(Sample_Block=='inc','Sample Group: lowest income quartile', 
                                                            Sample_Block=='pop','Sample Group: lowest density quartile',
...
#| echo: false
#| output: false 
# this will take a while but it is faster than the free servers 
...
pri<-distance_sample[V1 != 'all',][
  ...
  text_replace(
   ...
   text_replace(
    pattern='pop',replacement='lowest density quartile') |>
   text_replace(
...
```{r}
distance_compare[,.(
    mean =mean(distance_difference, na.rm = TRUE),
    median = median(distance_difference, na.rm = TRUE),
    sd = sd(distance_difference, na.rm = TRUE),
    min = min(distance_difference, na.rm = TRUE),
    max = max(distance_difference, na.rm = TRUE)
), by=.(grp_label)] |>  gt() |> 
  tab_header(
    title = "Descriptive Statistics of Distance Difference (m) by Sample Group"
  ) |>
  cols_label(
    grp_label= "Sample Group",
    mean = "Mean",
    median = "Median",
  ...
  text_replace(
    pattern='Sample Group: ',replacement='') 
```
...
axis(2, at = 4:1, labels = c("density","income", "white", "remaining"), las = 1, cex.axis = 0.8)
```

# 

```{r, resuls="asis"}
#| label: Select the cases with the bigest difference 
...
biggest_distances<-biggest_distances[,.(row = .I,grp_label,GMAP_orig, OSM_orig, GMAP_dest,OSM_dest,GMAP_distance_m,OSM_distance_m,distance_difference)] |> 
  melt(id.vars=c("row","grp_label")) 
biggest_distances <- biggest_distances[,':='(
  program = fcase(
                grepl("GMAP_",variable), 'GMAP' ,
                grepl("OSM_",variable), 'OSM',
                default = 'Difference'),
  variable = gsub("(GMAP_|OSM_)","",variable)
  )]
...
```{r}
#| label: difference in route selection

osm_maps= apply(records_2explore[, .(orig_lon,orig_lat,dest_lon,dest_lat)], 1, function(x) {make_osrm_map(x[1], x[2], x[3], x[4])})

g_maps  = apply(records_2explore[, .(orig_lat,orig_lon,dest_lat,dest_lon)], 1, function(x) 
                                                                          {make_google_map(x[1], x[2], x[3], x[4], getOption("GMAP_Directions_API_KEY"))})
```
