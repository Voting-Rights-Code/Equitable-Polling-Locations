...
read_config <- function(config_folder){
	config_file_list <- paste(config_folder, list.files(config_folder), sep = '/')
	config_list <- lapply(config_file_list, read_yaml)
	return(config_list)
}
...
combine_results<- function(location, config_folder, result_type, variable_field){
	#read in config 
