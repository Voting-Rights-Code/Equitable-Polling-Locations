...
# TEMPOTARY FIX - change 'year' variable from string to numeric
config.year = [2022]
config_df = config.df()
...
demographic_res = pd.read_csv('BigQuery_integration_test/York_SC_original_configs_collated/York_SC_original_configs_residence_distances.csv')
result_df = pd.read_csv('BigQuery_integration_test/York_SC_original_configs_collated/York_SC_original_configs_result.csv')
...
# # Upload
print(config.year)
print(config_df['year'])
print(type(config.year[0]))
print(type(config_df['year']))
print(type(config_df['year'][0]))
