...
def demographic_domain_summary(result_df, domain):
    ...
    demographic_all = pd.melt(
        result_df[[domain, "distance", "population", "white", "black", "native", "asian", "hispanic"]],
        id_vars=[domain, "distance"],
    ...
    demographic_all["weighted_dist"] = demographic_all["demo_pop"] * demographic_all["distance"]
    ...
    demographic_dist = (
        demographic_all[[domain, "demographic", "weighted_dist"]].groupby([domain, "demographic"]).agg("sum")
    )
...
def demographic_summary(demographic_df, result_df, beta, alpha):
    ...
    if beta != 0:
        # add the distance column back in from dist_df
        ...
        distances = result_df[["id_orig", "distance"]].set_index("id_orig")
        ...
        demographics["KP_factor"] = math.e ** (-beta * alpha * demographics["distance"])
        ...
        demographic_ede = (
            demographics[["demographic", "demo_res_obj_summand", "demo_pop"]].groupby("demographic").agg("sum")
        )
...
def compute_kp_score(df, beta, *, alpha=None, population_column_name="population", distance_column_name="distance"):
    distance_df = df[[population_column_name, distance_column_name]].copy()
    distance_df.columns = ["population", "distance"]
    if beta == 0:
