...
from equitable_locations.common.model_data import get_max_min_dist

from equitable_locations.common.utils import timer
    ...
    if site_penalty:
        ...
        def obj_rule_0(model):
            average_weighted_distances = sum(
                model.matching[pair] * model.weighted_dist[pair] for pair in model.pairs
            ) / total_pop + sum(model.open[site] * site_penalty for site in model.penalized_sites)
            return average_weighted_distances

        def obj_rule_not_0(model):
            average_weighted_distances = (
                sum(model.population[pair[0]] * model.matching[pair] * model.KP_factor[pair] for pair in model.pairs)
                / total_pop
        ...
        def obj_rule_0(model):
            ...
            average_weighted_distances = (
                sum(model.matching[pair] * model.weighted_dist[pair] for pair in model.pairs) / total_pop
            )
            return average_weighted_distances
...
def build_max_new_rule(
    ...
    def max_new_rule(
        ...
        if not any(model.new_locations[precincts] for precincts in model.precincts):
            ...
            return (
                sum(model.open[precinct] * model.new_locations[precinct] for precinct in model.precincts)
...
def build_min_old_rule(
    ...
    def min_old_rule(
        ...
        if config.minpctold == 0:
            ...
            return (
                sum(model.open[precinct] * (1 - model.new_locations[precinct]) for precinct in model.precincts)
...
def build_capacity_rule(
    ...
    def capacity_rule(
        ...
        return sum(
            model.population[res] * model.matching[res, precinct] for res in model.within_precinct_radius[precinct]
...
def compute_kp_factor(config: PollingModelConfig, alpha: float, dist_df):
    return math.e ** (-config.beta * alpha * dist_df["distance"])
...
def polling_model_factory(
    ...
    *,
    ...
    model.distance = pyo.Param(
        model.pairs, initialize=dist_df[["id_orig", "id_dest", "distance"]].set_index(["id_orig", "id_dest"])
    )
    # population weighted distances
    model.weighted_dist = pyo.Param(
        model.pairs, initialize=dist_df[["id_orig", "id_dest", "weighted_dist"]].set_index(["id_orig", "id_dest"])
    )

    # KP factor
    dist_df["KP_factor"] = compute_kp_factor(config, alpha, dist_df)
    # math.e**(-config.beta*alpha*dist_df['distance'])
    max_KP_factor = dist_df.groupby("id_orig")["KP_factor"].agg("max").max()
    if max_KP_factor > 9e19:
        warnings.warn(
            f"Max KP_factor is {max_KP_factor}. SCIP can only handle values up to {1e20}. Consider a less negative value of beta."
        )
    model.KP_factor = pyo.Param(
        model.pairs, initialize=dist_df[["id_orig", "id_dest", "KP_factor"]].set_index(["id_orig", "id_dest"])
    )
    # new location marker
    ...
    obj_rule = build_objective_rule(
        config, total_pop, alpha, site_penalty=site_penalty, kp_penalty_parameter=kp_penalty_parameter
    )
