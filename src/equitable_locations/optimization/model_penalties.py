...
from equitable_locations.optimization.model_solver import solve_model
from equitable_locations.optimization.model_factory import polling_model_factory
from equitable_locations.optimization.model_results import incorporate_result, compute_kp_score
...
def incorporate_penalties(
    dist_df, alpha, run_prefix, result_df, ea_model, config: PollingModelConfig, log: bool = False
):
    ...
    ea_model_penalized = polling_model_factory(
        dist_df, alpha, config, exclude_penalized_sites=False, site_penalty=penalty, kp_penalty_parameter=kp1
    )
    if log:
