...
3. **R Setup**:
   - Install required R packages:
    ...
    install.packages(c("data.table", "scales", "knitr", "gt", 
                        "gtsummary", "leaflet", "osrm", "sf", 
                        "httr2", "jsonlite", "xml2", "mapsapi",
                        "ggmap"
                    ))
    ```
...
1. ### Google Map API Key
    ...
    The program requires an API key that has been enabled for the Google Distance Matrix API and the Google Directions API.  The key should be stored in a file called `authentication_files` which is located in the root.  The program will automatically look for this file and privately store the key.

2. ### Selecting The Geographic Area To Be Processed

    The program should be run from the root directory.   It provides the user with a basic interface to select the directory containing the {location}driving_distance.csv file.
...
5. ### Google Driving Distances
    ...
    $$
    program\_distance\_m = Distance _{osm}  - Distance_{gmap}
    $$
...
1. ### Google Map API Key
    ...
    This program requires an API key that has been enabled for the Google Distance Matrix API and the Google Directions API.    The key should be stored in a file called `authentication_files` which is located in the root.  The exact path to the program root must be specified in the `#define data files` section of the code.   Once defined the code will automatically look for this file and privately store the key.
...
- Batch processing of Google driving distances to avoid API key suspension.
