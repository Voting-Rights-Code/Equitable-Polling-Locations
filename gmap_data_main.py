...
def df_sample(df: pd.DataFrame, quartile: float):
    ...
    sample = df.groupby("Sample", group_keys=False).apply(
        lambda x: x.sample(SAMPLE_ROWS // group_rows)
    )
...
def helper(dest_lat: float, dest_lon: float, orig_lat: float, orig_lon: float):
    ...
    distance_matrix = pd.read_json(f"https://maps.googleapis.com/maps/api/distancematrix/json?units=metric&destinations={dest_lat}%2C{dest_lon}&origins={orig_lat}%2C{orig_lon}&mode=driving&key={GMAP_api_key}", orient='columns')
